...
EstimationNode::EstimationNode()
{
	...
	svcl_ardrone_automation_pub	   = nh_.advertise<std_msgs::String>(command_channel,50);
	svcl_ardrone_automation_sub	   = nh_.subscribe(command_channel,50, &EstimationNode::comCb, this);
...
void EstimationNode::trackerCb(const svcl_ardrone_automation::tracker& msg)
{
	...
	loc = msg.loc;
    cout<<"loc: " << loc[0] << loc[1]<<loc[2]<<" "<<loc[3] <<"\n";
	yaw = msg.yaw;
    cout<<"yaw: "<<yaw[0]<<" "<<yaw[1]<<"\n";
...
void EstimationNode::Loop()
{
	  ...
	  while (nh_.ok())
	  {
		  ...
		  pthread_mutex_lock( &filter->filter_CS );
		  svcl_ardrone_automation::filter_state s = filter->getPoseAt(ros::Time().now() + predTime);
		  pthread_mutex_unlock( &filter->filter_CS );
...
void EstimationNode::dynConfCb(svcl_ardrone_automation::StateestimationParamsConfig &config, uint32_t level)
{
...
pthread_mutex_t EstimationNode::svcl_ardrone_automation_CS = PTHREAD_MUTEX_INITIALIZER; //pthread_mutex_lock( &cs_mutex );
void EstimationNode::publishCommand(std::string c)
{
	std_msgs::String s;
	s.data = c.c_str();
	pthread_mutex_lock(&svcl_ardrone_automation_CS);
	svcl_ardrone_automation_pub.publish(s);
	pthread_mutex_unlock(&svcl_ardrone_automation_CS);
}
