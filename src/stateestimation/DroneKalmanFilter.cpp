...
svcl_ardrone_automation::filter_state DroneKalmanFilter::getCurrentPoseSpeed()
{
	svcl_ardrone_automation::filter_state s;
	s.x = x.state[0];
	s.y = y.state[0];
	s.z = z.state[0];
	s.yaw = yaw.state[0];
...
svcl_ardrone_automation::filter_state DroneKalmanFilter::getPoseAt(ros::Time t, bool useControlGains)
{
	// make shallow copy
